...
import { TioDto } from '../commons/request/tioDto';
...
export class TokenService {
  myApiUrl = "http://localhost:3333/api/v1/usuarios/";
  KEYTOKEN = "KEYTOKEN";
  ...
  async listaSinToken() {
    var response;
    try{
        response = await this.httpClient.get<Tio[]>(this.myApiUrl).toPromise();
        //response = await this.httpClient.get<Tio[]>(this.tioURL + 'lista').toPromise();        
        console.log('response');
        console.log(JSON.stringify(response));
      }catch(e: unknown | any){
  ...
  async lista() {
    ...
    try{
        ...
        response = await this.httpClient.get<Tio[]>(this.myApiUrl, requestOptions).toPromise();
        //response = await this.httpClient.get<Tio[]>(this.tioURL + 'lista').toPromise();        
        console.log('response');
        console.log(JSON.stringify(response));
      }catch(e: unknown | any){
